...
if($action == 'ajax'){
	...
	if (isset($_REQUEST['id_tmp_paynote'])){
		...
		$q = "DELETE FROM tbl_tmp_rend WHERE id_tmp_paynote = '$id'";
        $res = $con->query($q);
	}
	...
	if (isset($_POST['tmp_description'])){
		...
		$insert ="INSERT INTO tbl_tmp_rend (agent_designated, tmp_description, tmp_quantity, tmp_amount) VALUES ('$agent','$desc','$qty','$amount')";
	...
	$q = "SELECT * FROM tbl_tmp_rend WHERE agent_designated = '". $_SESSION['user_system'] ."'";
    $res = $con->query($q);
	...
	<tr>
		...
		<td>
		<?php echo $row['tmp_description'];?></td>
