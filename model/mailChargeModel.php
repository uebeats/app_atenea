<?php
    ...
    $number = $_GET['number'];
    $q = "SELECT * FROM tbl_chargenote_system WHERE number_chargenote = '$number'";
    $rs = $con->query($q);
            ...
            <div id="content">
                ...
                <div class="page" style="font-size: 7pt">
                    ...
                    <table style="width: 100%; font-size: 12px;padding: 10px 0 10px 0" class="header">
                        <tbody>
                            <tr>
                                <td>Cliente: <b><?php echo $rw_owner['name_owner'];?></b></td>
                                <td>Agente: <b><?php echo $rw_paynote['agent_admin'];?></b></td>
                            </tr>

                            <tr>
                                <td>Rut Cliente: <strong><?php echo $rw_owner['rut_owner'];?></strong></td>
                    ...
                    <table class="change_order_history">
                        ...
                        <tbody style="font-size: 12px">
                            <tr>
                                <th colspan="4" style="text-align: center">Historial de Últimos 5 Cobros</th>
                            </tr>
                            <?php
                            ...
                            ?>
                            <tr class="even_row">
                                <td class="change_order_unit_col" style="border-left-style: none;text-align: center;">N° <?php echo $r_paynote['number_chargenote'];?></td>
                                ...
                                <td class="change_order_unit_col" style="border-left-style: none;text-align: center;"><?php echo $r_paynote['status_chargenote'];?></td>
                            </tr>
                            <?php
                            }
                            ?>
                    ...
                    <table class="change_order_history" style="width: 100%; border-bottom: 1px solid black; border-top: 1px solid black;font-size: 12px;">
                        <tbody>
                            <tr>
                                <td colspan="4" style="font-size: 12px;"><h2 style="margin-bottom: 0">Forma de Pago: <?php echo $rw_paynote['pago_chargenote'];?></h2>
                                El monto total mostrado en esta nota de cobro debe ser integramente abonado a la cuenta bancaria que se indica a continuación:</td>
                            </tr>
                        </tbody>
                        <tbody>
                            <tr>
                    ...
                    <table style="width: 100%; font-size: 12px;padding-top:40px;text-align: center;">
                        <tbody>
                            <tr>
                                <td>Nota de Cobro emitida por <strong><?php echo $rw_paynote['agent_designated'];?></strong></td>
                            </tr>
                        </tbody>
                    </table>
...
<?php
    ...
    try {
        ...
        $mail->addAttachment($filename, 'Nota de Cobro');            // Optional name

        // Content
        $number = $_GET['number'];
        $q = "SELECT name_owner FROM tbl_chargenote_system WHERE number_chargenote = '$number'";
        $res = $con->query($q);
        $row = $res->fetch_assoc();
        ...
        $mail->Body    = '<b>Estimado '.$name_email.',</b><br>Junto con saludar, le indicamos que hemos adjuntado a este correo electronico la Nota de Cobro N°'.$number.'.<br> Si tiene alguna duda, consulta o sugerencia, puede responder a este correo electronico o llamar a los telefonos indicados en la firma.<br><small>************ Si usted ya cancelo lo adeudado, favor omitir esta informacion ************</small><br><br>
